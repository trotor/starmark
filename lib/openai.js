...
export class OpenAIClient {
  ...
  async _addTokens(usage) {
    if (!usage) {
      console.warn('[Starmark] _addTokens called with no usage data');
      return;
    }

    console.log('[Starmark] _addTokens called with:', JSON.stringify(usage));

    try {
      const current = await this.getTokenUsage();
      console.log('[Starmark] Current token usage:', JSON.stringify(current));
      ...
      console.log('[Starmark] Saving updated token usage:', JSON.stringify(updated));
      await chrome.storage.session.set({ tokenUsage: updated });

      // Verify it was saved
      const verify = await chrome.storage.session.get('tokenUsage');
      console.log('[Starmark] Verified saved token usage:', JSON.stringify(verify));
    } catch (error) {
...
Respond with JSON only.`;
    ...
    try {
      ...
      // Track token usage
      console.log('[Starmark] Deep classify API response usage:', JSON.stringify(data.usage));
      if (data.usage) {
...
Classify this bookmark. JSON only.`;
    ...
    try {
      ...
      // Track token usage
      console.log('[Starmark] Quick classify API response usage:', JSON.stringify(data.usage));
      if (data.usage) {
        await this._addTokens(data.usage);
      }
